...
## Ask from Inside the Console

For most product requests, the console is the fastest route — the request is recorded with the context we need, and you can track the reply on the **My requests** page.

| What you need | Where to ask |
| :-- | :-- |
| A GPU that is sold out | Join the waitlist from the GPU selection |
| A GPU model we don't offer | **Request a GPU** from the GPU selection |
| A framework or image that isn't a template | **Request a template** from the template picker |
| A model for Air API | **Request a model** from the Air API model list |
| More replicas | **Request limit increase** from the replica settings |
| A long-term rental | **Rental inquiry** from Long-term Rentals |

See [Requests and Waitlist](/docs/console/requests) for how requests are tracked and answered.
