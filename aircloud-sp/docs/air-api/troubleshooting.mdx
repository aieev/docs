...
<AccordionGroup>
  <Accordion title="402 Payment Required — Insufficient credits">
    **Cause:** Your Organization's credit balance has been exhausted, preventing further API requests.
...
<AccordionGroup>
  <Accordion title="429 Too Many Requests — Rate limit exceeded">
    ...
    **Resolution:**
    ...
    <Note>
      Air API does not enforce a published per-account request or token rate limit today. A 429 means the model's serving capacity is saturated at that moment, so backing off and retrying is the right response.
    </Note>
  </Accordion>
  ...
  <Accordion title="502 Bad Gateway / 503 Service Unavailable">
    **Cause:** The model service is temporarily unavailable or undergoing maintenance.
    ...
    **Resolution:**
    ...
    <Note>
      Planned maintenance and incidents are announced through console notifications. See [Notifications](/docs/console/notifications).
    </Note>
  </Accordion>
...
<AccordionGroup>
  ...
  <Accordion title="403 Forbidden — Access to the model is denied">
    ...
    **Resolution:**
    ...
    - An API key can only reach the Air API endpoints selected when the key was created. Check the key's **Air API Access** setting in the Console, or create a new key that includes the model you need.
    - Some models are enabled per organization. If the model does not appear in your Console at all, request it through the [Contact and Feedback](/docs/support/contact-and-feedback) page.
  </Accordion>
</AccordionGroup>
