...
For example, for Qwen3.5-35B-A3B:

```text
https://ap-1.aieev.cloud/endpoints/qwen3-5-35b-a3b/v1
```
...
| :-- | :-- | :-- | :-: | :-: | :-: | :-: | :-: |
| Qwen3.5-35B-A3B | `qwen3-5-35b-a3b` | `qwen/qwen3.5-35b-a3b` | 256K | ✅ | ✅ | ✅ up to 4 | ✅ |
...
| Qwen3-TTS | *(separate endpoint)* | `qwen/qwen3-tts-customvoice` | - | - | ❌ | ❌ | ❌ |
...
<Note>
  Streaming is currently supported on **Qwen3.5-35B-A3B**, **Qwen3.5-9B**, and **Llama3.3-70B**.
</Note>

## Tool Calling (Function Calling)

Lets the model call external functions or services. Supported on **Qwen3.5-35B-A3B**, **Qwen3.5-9B**, and **Llama3.3-70B**.

Tool calling is a three-step flow. The model does not execute functions itself — it **suggests which function to call and with what arguments**; your application executes it and sends the result back.
...
The model can return multiple function calls in a single response via the `tool_calls` array. For example, asking "What's the weather in Seoul and Busan?" returns two `get_weather` calls together. Control this with the `parallel_tool_calls` parameter (default `true`).

```python
response = client.chat.completions.create(
    model="qwen/qwen3.5-35b-a3b",
    messages=[{"role": "user", "content": "What's the weather in Seoul and Busan?"}],
    tools=tools,
    parallel_tool_calls=False,  # limit to one function call at a time
)
```

<Warning>
  **Do not use `parallel_tool_calls: false` with Llama3.3-70B.** It returns an empty `tool_calls` array and dumps the function calls as plain text in the response body.
</Warning>

### Streaming tool calls

With `stream: true`, a tool call is delivered across multiple chunks.

- Each tool call starts with a lead chunk carrying `id`, `type`, and `function.name`; subsequent chunks carry only `function.arguments` fragments (`id` and `name` are null).
- Concatenate the fragments by `tool_calls[].index` to reconstruct the complete JSON arguments.
- Multiple tool calls stream sequentially by index (no interleaving), and the stream ends with `finish_reason: "tool_calls"`.
- On Qwen3.5 models, reasoning chunks (`delta.reasoning`) stream first, followed by the tool call chunks.
...
Qwen3.5 chat models accept image input. Pass `content` as an array and include images with the `image_url` type. Both public URLs and base64 encoding are supported.
...
<Note>
  The number of images per request is limited per model — **Qwen3.5-35B-A3B: up to 4**, **Qwen3.5-9B: up to 1**. Exceeding the limit returns a 400 error. Video input is not supported.
</Note>

## Reasoning

Qwen3.5 chat models support reasoning (thinking). The reasoning trace is returned separately from the final answer (`content`) in the `reasoning` field.
...
- Reasoning text also consumes `max_tokens`. If the value is too small, the model may spend the entire budget on reasoning and the answer gets cut off (`finish_reason: "length"` with an empty `content`) — set it generously.
- When streaming, reasoning fragments arrive first via `delta.reasoning`, followed by the final answer via `delta.content`.
- Show only `content` to end users; use `reasoning` for debugging and logging. You also don't need to include it in the conversation history (`messages`) of subsequent requests.
...
<Tip>
  When using JSON mode, also instruct the model to respond in JSON in your prompt — including the fields you expect — for more reliable results. On Qwen3.5 models, reasoning consumes tokens and can truncate the JSON — combine JSON mode with [disabled reasoning](#disabling-reasoning) or set `max_tokens` generously.
</Tip>

## Embedding

Converts text into vectors, useful for semantic search, recommendation systems, and RAG. Pass an array of strings as `input` to embed multiple texts in a single request.
...
for item in response.data:
    print(item.index, len(item.embedding))  # 0 4096 / 1 4096
```
