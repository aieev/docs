...
With `stream: true`, tool calls arrive split across chunks: the first chunk carries `id` and `function.name`, and later chunks carry `function.arguments` fragments. Accumulate the fragments by `index`; the stream ends with `finish_reason: "tool_calls"`.
