...
| :-- | :-- | :-- | :-: | :-: | :-: | :-: | :-: |
| Qwen3.8-27B | `qwen-3-8-27b` | `qwen/qwen3.8-27b` | 256K | ✅ | ✅ | ✅ image + video | ✅ |
...
<Note>
  Streaming is supported on every chat model: **Qwen3.8-27B**, **Qwen3.6-35B-A3B**, **Qwen3.5-9B**, **Qwen3-32B**, **Llama3.3-70B**, and **DeepSeek V4 Flash**.
</Note>

## Tool Calling (Function Calling)

Lets the model call external functions or services. Supported on **Qwen3.8-27B**, **Qwen3.6-35B-A3B**, **Qwen3.5-9B**, **Qwen3-32B**, **Llama3.3-70B**, and **DeepSeek V4 Flash**.
...
Qwen3.8-27B, Qwen3.6-35B-A3B, and Qwen3.5-9B accept image input. Qwen3.8-27B also accepts video, passed as a `video_url` block. Pass `messages[].content` as an array and include images with the `image_url` type. `image_url.url` accepts an externally reachable image URL or a base64-encoded data URI (`data:image/jpeg;base64,...`).
...
<Note>
  The number of images per request is limited per model — **Qwen3.6-35B-A3B: up to 4**, **Qwen3.5-9B: up to 1**. Video input is supported on **Qwen3.8-27B** only.
</Note>

## Reasoning

Qwen chat models and DeepSeek V4 Flash support reasoning (thinking). On **Qwen3.8-27B** the depth is tunable with `reasoning_effort` (`xhigh` / `medium` / `low`), and streamed reasoning arrives in `reasoning_content` rather than `reasoning`. The reasoning trace is returned separately from the final answer (`content`) in the `reasoning` field.
