...
Qwen3.5 chat models accept image input. Pass `content` as an array and include images with the `image_url` type. Both public URLs and base64 data URIs (`data:image/jpeg;base64,...`) are supported.
...
<Note>
  The number of images per request is limited per model — **Qwen3.5-35B-A3B: up to 4**, **Qwen3.5-9B: up to 1**. Video input is not supported.
</Note>
