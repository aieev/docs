...
<Note>
  The table above lists the GPU instance types generally available today. Additional types — including NPU and CPU-tier instances — are enabled per organization and appear in the container create flow only when your organization has access. The list in the console is always the authoritative one.
</Note>

Renting a GPU long term, starting from one month, lowers the hourly rate. See [Long-term Rentals](/docs/console/long-term-rentals) for terms and payment, and the [Pricing page](https://www.aieev.com/pricing) for the latest rates.
