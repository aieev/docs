...
예를 들어 Qwen3.5-35B-A3B는 다음과 같습니다.
...
| :-- | :-- | :-- | :-: | :-: | :-: | :-: | :-: |
| Qwen3.5-35B-A3B | `qwen3-5-35b-a3b` | `qwen/qwen3.5-35b-a3b` | 256K | ✅ | ✅ | ✅ 최대 4장 | ✅ |
...
| Qwen3-TTS | *(별도 엔드포인트)* | `qwen/qwen3-tts-customvoice` | - | - | ❌ | ❌ | ❌ |
...
- 각 청크는 `choices[0].delta.content`에 텍스트 조각을 담아 전달되며, 스트림이 끝나면 `data: [DONE]` 이벤트가 전송됩니다.
- `stream_options={"include_usage": true}`를 함께 보내면 마지막 청크에서 토큰 사용량(`usage`)을 확인할 수 있습니다.

<Note>
  현재 Streaming은 **Qwen3.5-35B-A3B**, **Qwen3.5-9B**, **Llama3.3-70B** 모델에서 지원됩니다.
</Note>

## Tool Calling (Function Calling)

AI가 외부 함수나 서비스를 호출할 수 있도록 하는 기능입니다. **Qwen3.5-35B-A3B**, **Qwen3.5-9B**, **Llama3.3-70B** 모델에서 지원됩니다.

Tool Calling은 세 단계로 진행됩니다. 모델이 함수를 직접 실행하는 것이 아니라, **어떤 함수를 어떤 인자로 호출할지 제안**하면 애플리케이션이 실행하고 그 결과를 다시 전달합니다.
...
모델은 한 응답에서 여러 함수 호출을 `tool_calls` 배열로 반환할 수 있습니다. 예를 들어 "서울과 부산의 날씨를 각각 알려줘"라고 요청하면 `get_weather` 호출 2개가 함께 반환됩니다. `parallel_tool_calls` 파라미터(기본값 `true`)로 이를 제어할 수 있습니다.

```python
response = client.chat.completions.create(
    model="qwen/qwen3.5-35b-a3b",
    messages=[{"role": "user", "content": "서울과 부산의 날씨를 각각 알려줘"}],
    tools=tools,
    parallel_tool_calls=False,  # 함수 호출을 한 번에 1개로 제한
)
```

<Warning>
  **Llama3.3-70B에서는 `parallel_tool_calls: false`를 사용하지 마세요.** `tool_calls`가 빈 배열로 반환되고 함수 호출 내용이 본문 텍스트로 출력되는 문제가 확인되었습니다.
</Warning>

### Streaming으로 Tool Call 받기

`stream: true`에서는 tool call이 여러 청크에 나뉘어 전달됩니다.

- 각 tool call은 `id`·`type`·`function.name`이 담긴 첫 청크로 시작하고, 이후 청크에는 `function.arguments` 조각만 담겨 옵니다(`id`·`name`은 null).
- 조각을 `tool_calls[].index` 기준으로 이어 붙여야 완전한 JSON 인자가 됩니다.
- 복수 함수 호출은 index별로 순차 전달되며(교차 없음), 스트림은 `finish_reason: "tool_calls"`로 끝납니다.
- Qwen3.5 모델은 추론 청크(`delta.reasoning`)가 먼저 전달된 뒤 tool call 청크가 이어집니다.
...
Qwen3.5 chat 모델은 이미지 입력을 지원합니다. `content`를 배열로 구성하고 `image_url` 타입으로 이미지를 전달하세요. 공개 URL과 base64 인코딩 모두 사용할 수 있습니다.
...
<Note>
  요청당 이미지 수는 모델별로 제한됩니다 — **Qwen3.5-35B-A3B: 최대 4장**, **Qwen3.5-9B: 최대 1장**. 한도를 초과하면 400 에러가 반환됩니다. 동영상 입력은 지원되지 않습니다.
</Note>

## Reasoning (추론 출력)

Qwen3.5 chat 모델은 추론(thinking)을 지원합니다. 추론 과정은 최종 답변(`content`)과 분리되어 `reasoning` 필드로 반환됩니다.
...
- 추론 텍스트도 `max_tokens`를 함께 소모합니다. 값이 너무 작으면 추론만 하다가 답변이 잘릴 수 있으므로(`finish_reason: "length"`, `content`가 비어 있음) 넉넉하게 지정하세요.
- 스트리밍에서는 `delta.reasoning`으로 추론 조각이 먼저 전달된 뒤 `delta.content`로 최종 답변이 이어집니다.
- 사용자에게는 일반적으로 `content`만 보여주고, `reasoning`은 디버깅·로깅 용도로 활용하세요. 이후 요청의 대화 이력(`messages`)에 포함할 필요도 없습니다.
...
<Tip>
  JSON 모드를 사용할 때는 프롬프트에도 "JSON으로 응답하라"는 지시와 원하는 필드 구성을 함께 명시하면 더 안정적인 결과를 얻을 수 있습니다. Qwen3.5 모델에서는 추론이 토큰을 소모해 JSON이 잘릴 수 있으니, [추론 비활성화](#추론-비활성화)와 함께 사용하거나 `max_tokens`를 넉넉히 지정하세요.
</Tip>

## Embedding

텍스트를 벡터로 변환하는 기능입니다. 시맨틱 검색, 추천 시스템, RAG 등에 활용할 수 있습니다. `input`에 문자열 배열을 전달하면 여러 텍스트를 한 번에 임베딩할 수 있습니다.
