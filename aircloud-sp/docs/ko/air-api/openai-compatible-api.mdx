...
`stream: true`에서는 tool call이 여러 청크에 나뉘어 전달됩니다. 첫 청크에 `id`와 `function.name`이 담기고, 이후 청크에는 `function.arguments` 단편이 이어집니다. 단편을 `index` 기준으로 이어 붙이면 되고 스트림은 `finish_reason: "tool_calls"`로 끝납니다.
