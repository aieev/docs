...
Qwen3.5 chat 모델은 이미지 입력을 지원합니다. `content`를 배열로 구성하고 `image_url` 타입으로 이미지를 전달하세요. 공개 URL과 base64 데이터 URI(`data:image/jpeg;base64,...`) 모두 사용할 수 있습니다.
...
<Note>
  요청당 이미지 수는 모델별로 제한됩니다 — **Qwen3.5-35B-A3B: 최대 4장**, **Qwen3.5-9B: 최대 1장**. 동영상 입력은 지원되지 않습니다.
</Note>
...
- 스트리밍에서는 추론 조각이 `delta.reasoning`으로 먼저 전달된 뒤 `delta.content`가 이어집니다.
