...
모델은 한 응답에서 여러 함수 호출을 `tool_calls` 배열로 반환할 수 있습니다. `parallel_tool_calls`를 `false`로 지정하면(기본값 `true`) 함수 호출을 한 번에 1개로 제한할 수 있습니다.

### Streaming으로 Tool Call 받기

`stream: true`에서는 tool call이 여러 청크에 나뉘어 전달됩니다. 첫 청크에 `id`와 `function.name`이 담기고, 이후 청크에는 `function.arguments` 조각이 이어집니다. 조각을 `index` 기준으로 이어 붙이면 되고, 스트림은 `finish_reason: "tool_calls"`로 끝납니다.
