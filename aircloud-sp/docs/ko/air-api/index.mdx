...
![프로젝트 개요에서 Air API 선택](/images/air-api/select-air-api.png)

### 2. 사용할 모델 선택

API 워크로드를 생성하고 필요한 모델을 선택합니다. 필요한 모델이 목록에 없는 경우 피드백을 통해 요청할 수 있습니다.

![모델 선택 화면](/images/air-api/select-model.png)
...
<Warning>
  시크릿 키는 생성 이후 다시 확인할 수 없습니다. 반드시 복사해 안전하게 보관하세요. 분실한 경우 재발급이 필요합니다.
</Warning>

![시크릿 키 발급 화면](/images/air-api/generate-key.png)

### 4. 코드 샘플로 연결 테스트

키를 발급받은 뒤 코드 샘플 탭으로 이동해 `your-api-key` 위치에 시크릿 키를 입력하고 연결 테스트를 진행합니다.

![코드 샘플 테스트](/images/air-api/code-sample.png)
...
![Playground 화면](/images/air-api/playground.png)
