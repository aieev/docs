...
## Accurate payment cancellation

**Cancelling unused card payments**

- **Exact eligibility**: Whether a payment can be cancelled is now decided by **whether the credits granted by that payment are still untouched**, not by the organization's total balance. A payment whose credits were used even partially cannot be cancelled; an unused one can, regardless of how long ago it was made.
- **No 7-day limit**: The time window for cancellation is gone.
...
- **Reasons shown**: In the cancellation dialog, hover over a payment that cannot be cancelled to see why.

**Before deleting an organization**

- The organization deletion dialog (including a last owner's account withdrawal) now shows how many of your own card payments can still be cancelled, with a direct link to Payment History.
...
GPU reservations are now called **long-term rentals**, and the whole flow lives in the console.

- **From one month**: The minimum term dropped from three months to one, with presets up to a year and a lower hourly rate the longer you rent.
...
**Requests, in the place you get stuck**

Ask for what you need from the screen where you needed it, instead of opening a separate ticket.
...
**GPU availability and resource requests**

Check GPU stock before you deploy, and request more resources right from the console when you hit a limit.

- **Availability at a glance**: The GPU list in the container create flow now shows an availability level per instance type, so you know what can be deployed before you try.
...
Spin up a container with the same configuration without setting everything up again.
...
**Revamped usage & billing experience**

It's now much clearer what you were charged and why.
...
Stay on top of your containers and billing with more granular notifications.
...
<CardGroup cols={2}>
  <Card title="AirCloud CLI Reference" href="/docs/air-container/use-aircloud-cli" icon="terminal">
    Learn how to install and use the AirCloud CLI.
  </Card>
  <Card title="Web Terminal and SSH Guide" href="/docs/air-container/access-and-connect" icon="display">
...
Three new models are now available through the [Air API Playground](https://ap-1.aieev.cloud:3000):
...
curl -X GET https://external.aieev.cloud:5000/external/api/v1/endpoints/{id} \
  -H "Authorization: Bearer YOUR_API_KEY"

# Scale replicas
curl -X POST https://external.aieev.cloud:5000/external/api/v1/endpoints/{id}/scale \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -d '{"replica_count": 3}'
```
